...
@user_router.get("/all", response_model=List[User], dependencies=[Depends(get_current_user)])
...
@user_router.delete("/{user_id}/hard", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(get_current_user)])
...
@user_router.patch("/{user_id}/restore", response_model=User, dependencies=[Depends(get_current_user)])
